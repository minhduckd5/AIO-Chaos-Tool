...
| 0:30–2:30 | Happy path | `demo-e2e.sh` or GUI Advisor wizard |
| 2:30–3:30 | **Resilience** | Stop Ollama → generate → fallback UNKNOWN → Scenarios: 0 → promote rejected |
...
> ChaosGen implements the advisor's research framework: filter real incidents via frequency-severity gatekeeper, describe unknowns structurally, promote to known catalog after human approval, then chaos-test and verify with accepted residual risk — closing the loop toward predictive maintenance.
